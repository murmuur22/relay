...
The installed startup path now loads protected opt-in configuration with explicit names/bind/TLS/fixed upstreams. Tests cover real configured HTTPS enrollment, custom-domain desktop launch, management-HTTP refusal, protected-input and certificate validation, upstream HTTPS/WSS identity checks, private-LAN binding, grants, End/restart revocation and unchanged config bytes. Backend tests use genuine temporary accounts and synthetic services; browser TLS uses a process-local fixture exception, not public browser trust.

Independent review initially found persisted native definitions could collide with a newly configured HTTPS authentication hostname. A separate fix added startup refusal before listeners/session publication/native startup, with three red-to-green migration regressions preserving account/config bytes. Final independent re-review found no concrete blocker and independently passed 33 backend/unit checks plus the custom-domain compiled-browser test. A programmatic port-zero equality regression was also reproduced and corrected; explicit equal nonzero ports remain rejected before state initialization.

Two full runs stalled in the existing updater desktop browser test under default parallel execution, while that test passed alone. Running the entire frontend suite serially passed; the standard `npm test` now uses bounded serial frontend execution without removing assertions, followed by a successful complete run. This avoids the reproduced concurrent-run stall; it is not a claim that its underlying browser/runtime cause was fixed. Builds retain the existing lazy-loaded bundle-size warning.

A clean Git-index export with fresh npm/uv dependencies passed both builds, all 148 backend and 68 frontend tests, real hybrid integration, and all updater suites. The staged-tree audit excluded private infrastructure values, credentials, local runtime/data, screenshots, historical spikes and uncleared fonts. Public fallback-font warnings are expected. Source is 0.6.0; GitHub publication, Linux/systemd and signed update evidence are recorded separately after execution. DNS/certificates/exposure remain operator responsibilities; private management access is still required for the independent updater. No production VM or network/trust configuration was changed.

## Local professor-demo tooling — 2026-09-22

Added a repeatable `npm run demo:jellyfin` launcher and opt-in `npm run demo:jellyfin:verify` acceptance test. The acceptance test was run red before the runner existed, then passed after implementation and on repeat invocation. Both headless and visible sandbox-enabled Chromium verification passed real nonadmin Relay/Jellyfin form logins, video/audio decoding, seek, minimize/restore identity, End denial of the old capability, fresh-login reopen and Close route/active-operation cleanup. Browser traffic was confined to desktop/app gateway origins. Actual screenshot and browser recording are under ignored `screenshots/jellyfin-demo/`; the screenshot was visually inspected.

`npm run build` passed with the existing lazy-loaded chunk-size warning. Full `npm test` passed on rerun: **133 backend, 65 frontend and real hybrid integration**. The first full run, concurrent with a separate headed browser verification, failed the existing lifecycle test's fixed-delay paused-state assertion; it is not counted as a pass or claimed fixed. The unchanged full rerun without the competing demo verification passed. No production code was changed for that failure.

This uses the already initialized isolated Jellyfin fixture and its synthetic 20-second clip, not a real household library or long-duration media qualification. The dedicated Chromium process uses an exact SPKI test exception; no system trust/hosts change or trusted Safari success is implied. The fallback recording is browser video without audio. No production deployment, publication, version bump or Linux qualification occurred. See `docs/jellyfin-demo.md` for launch/reset instructions and remaining portability requirements.

## Unreleased experimental normal-desktop gateway — local verification

Parent executed `npm run build`, full `npm test` and `npm run test:updater` on the final implementation: **133 backend, 65 frontend, actual hybrid integration, 60 Python updater, 6 updater unit and 35 updater browser tests passed**. The prior worker deadline interruption did not qualify its full run; the affected updater test and complete final suites subsequently passed under parent execution. Independent final re-review passed 20 focused gateway backend/browser tests, three additional held-save/restart browser regressions and the original cancellation-saturation reproducer.

Normal desktop Add app/editor/shortcut/window behavior is now tested with the opt-in local Gateway target configuration. End/reopen/Close/minimize semantics and delayed launch, layout-save and account-switch races have real compiled-browser coverage. Backend authority uses actual Relay accounts/sessions/grants. Review found and corrected globally shared End-cancellation saturation; per-session bounded bookkeeping now keeps another user's cleanup available. No generic native auth/cookie-host guard was relaxed.

`tools/verify-local-desktop-gateway.mjs` also passed actual local File Browser and Jellyfin workflows through the normal desktop, not the earlier proof launcher: nonadmin login, file upload/download byte equality, End invalidating old access, fresh-login reopen and Close cleanup; Jellyfin viewer video/audio decoding, seeking and End cleanup. The actual screenshot `screenshots/local-normal-desktop-jellyfin.png` was inspected and shows the synthetic test clip and End control. Containers were stopped afterward; generated test data remains isolated in local fixture volumes. No production libraries were used.

This is an **unreleased, disabled-by-default, loopback-only programmatic experiment**. Production source version remains0.5.0; no commit, push, release or deployment is implied. Domain/DNS/valid-certificate design, trusted Safari, Linux/load/large-media tests, broader retained-storage/handshake races and the experimental edge's updater handoff remain gates. Chromium used a process-local test SPKI exception; Node verified fixture CA/hostname. No trust-store/hosts change was performed in this stage. See `docs/experimental-desktop-gateway.md` for exact configuration and limits.
