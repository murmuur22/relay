...
test('browser fallback egress is denied even when a browser request misses interception',async()=>{
 ...
 try{browser=await launchBrowser();const context=await browser.newContext({serviceWorkers:'block'});await routeWeb(context,{address},transport);const page=await context.newPage();await page.goto(address);assert.equal(await page.textContent('h1'),'Fixture');const routedHits=hits;
 await context.unrouteAll({behavior:'wait'});const response=await page.goto(address+'unrouted',{timeout:2500}).catch(()=>null);assert.equal(hits,routedHits);assert.ok(!response||response.status()===403);
