...
export async function createGateway({port=4180,runtime=ROOT+'.runtime',native=false,keepsakesPort=4181,data=ROOT+'.data/keepsakes',sessionMs=8*60*60*1000}={}){
 ...
 const revoke=async s=>{if(!s||s.revoked)return;s.revoked=true;sessions.delete(s.id);clearTimeout(s.timer);for(const ws of s.sockets)ws.terminate();for(const res of s.responses)res.destroy();await s.manager.close();};
 ...
 const login=async(req,res,user)=>{
  ...
  const manager=new Manager(dir,accounts.apps(user));
  ...
  const s={id,userId:user.id,user,csrf:token(),expires:Date.now()+sessionMs,manager,sockets:new Set(),responses:new Set()};sessions.set(id,s);
 ...
 app.get('/api/session',(req,res)=>{const s=req.session;res.json({csrf:s.csrf,user:publicUser(s.user),apps:accounts.apps(s.user),windows:[...s.manager.windows.values()],limits:{maxStreams:2}});});
 ...
 app.post('/api/admin/services',wrap(async(req,res)=>{const service=await accounts.service(null,req.body||{});for(const s of sessions.values())s.manager.apps=accounts.apps(accounts.state.users.find(u=>u.id===s.userId));res.json(service);}));
 for(const method of ['patch','delete'])app[method]('/api/admin/services/:id',wrap(async(req,res)=>{
  ...
  const affected=[...sessions.values()].filter(s=>s.user.role!=='admin'&&accounts.allowed(s.user,service));
 ...
 app.get('/api/status',wrap(async(req,res)=>{
  ...
  const status={};for(const a of accounts.apps(req.session.user)){
   const r=req.session.manager.resources.get(a.id);
   status[a.id]=a.mode==='native'?{state:health[a.template]?'Online':'Offline',checkedAt:health.checkedAt,detail:'Native runtime readiness'}:{state:r?(r.page.isClosed()||r.failed?'Offline':'Online'):'Unknown',checkedAt:Date.now(),detail:r?'Synthetic browser readiness':'Synthetic template; no running page. Upstream health not measured.'};
  }res.json(status);
 }));
 ...
 return {origin,app,server,accounts,sessions,get manager(){return [...sessions.values()][0]?.manager;},nativeService,authorized:req=>!!getSession(req),close:async()=>{for(const s of [...sessions.values()])await revoke(s);await nativeService?.close();await new Promise(r=>server.close(r));}};
}
