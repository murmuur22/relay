...
export const publicUser=({id,username,displayName,role,grants,disabled,mustChange,preferences})=>({id,username,displayName,role,grants,disabled,mustChange,preferences:preferences??{showAppStatus:true}});
export class Accounts{
 ...
 async init(){
  try{this.state=JSON.parse(await readFile(this.runtime+'/accounts.json','utf8'));
   ...
   for(const user of this.state.users){if(!('preferences' in user))user.preferences={showAppStatus:true};if(!user.preferences||typeof user.preferences!=='object'||Array.isArray(user.preferences)||Object.keys(user.preferences).some(k=>k!=='showAppStatus')||typeof user.preferences.showAppStatus!=='boolean')throw Error('Invalid preferences');this.validateUser(user,this.state);if(typeof user.mustChange!=='boolean')throw Error('Invalid account state');}
   for(const service of this.state.services){if(service.kind==='web'){webConfig(service);if(!/^service-[0-9a-f-]{36}$/.test(service.id))throw Error('Invalid registry state');continue;}const template=this.templates.find(t=>t.id===service.template);if(!template||service.mode!==template.mode||service.url!==template.url||! /^(?:[a-z]+(?:-[a-z]+)*|service-[0-9a-f-]{36})$/.test(service.id)||typeof service.label!=='string'||!service.label.trim()||service.label.length>64||typeof service.enabled!=='boolean')throw Error('Invalid registry state');}
 ...
 async createUser(body){return this.mutate(async state=>{if(state.users.length>=64||state.users.some(u=>u.username===body.username))throw fail(400,'User unavailable');const user={id:randomUUID(),username:body.username,displayName:body.displayName||body.username,role:body.role||'user',grants:body.grants||[],disabled:false,mustChange:false,preferences:{showAppStatus:true},password:await hash(body.password)};this.validateUser(user,state);state.users.push(user);return publicUser(user);});}
 ...
 async preferences(id,body){if(!body||Object.keys(body).some(k=>k!=='showAppStatus')||typeof body.showAppStatus!=='boolean')throw fail(400,'Invalid preference');return this.mutate(state=>{const u=state.users.find(u=>u.id===id);u.preferences={showAppStatus:body.showAppStatus};return publicUser(u);});}
 async service(id,body,remove=false){return this.mutate(state=>{
  let service=state.services.find(s=>s.id===id);
  if(!remove&&(body.kind==='web'||service?.kind==='web')){
   if(Object.keys(body).some(k=>!['kind','mode','label','address','icon','openMode','allowedOrigins','userIds','enabled'].includes(k)))throw fail(400,'Invalid app fields');
  ...
  if(remove){if(!service)throw fail(404,'Service unavailable');state.services=state.services.filter(s=>s.id!==id);for(const user of state.users)user.grants=user.grants.filter(g=>g!==id);return {removed:true};}
 ...
 async enroll(password){return this.mutate(async state=>{if(state.users.length)throw fail(403,'Setup unavailable');const user={id:randomUUID(),username:'admin',displayName:'admin',role:'admin',grants:[],disabled:false,mustChange:false,preferences:{showAppStatus:true},password:await hash(password)};state.users.push(user);return user;});}
