...
export const publicUser=({id,username,displayName,role,grants,disabled,mustChange})=>({id,username,displayName,role,grants,disabled,mustChange});
export class Accounts{
 ...
 async createUser(body){return this.mutate(async state=>{if(state.users.length>=64||state.users.some(u=>u.username===body.username))throw fail(400,'User unavailable');const user={id:randomUUID(),username:body.username,displayName:body.displayName||body.username,role:body.role||'user',grants:body.grants||[],disabled:false,mustChange:false,password:await hash(body.password)};this.validateUser(user,state);state.users.push(user);return publicUser(user);});}
 ...
 async enroll(password){return this.mutate(async state=>{if(state.users.length)throw fail(403,'Setup unavailable');const user={id:randomUUID(),username:'admin',displayName:'admin',role:'admin',grants:[],disabled:false,mustChange:false,password:await hash(password)};state.users.push(user);return user;});}
