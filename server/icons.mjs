...
export function iconPython(){
 const python=process.env.RELAY_ICON_PYTHON ?? root+'integrations/keepsakes/.venv/bin/python';
 if(!isAbsolute(python))throw Error('RELAY_ICON_PYTHON must be an absolute path');
 return python;
}
let active=0;
export async function normalizeIcon(bytes,type){
 ...
 try{return await new Promise((resolve,reject)=>{
  const child=spawn(iconPython(),[root+'tools/icon-normalize.py',type],{stdio:['pipe','pipe','ignore']});let size=0,chunks=[],settled=false;
