<script>
 ...
 let {win,onactivate,onnavigation=()=>{}}=$props();
 ...
 function enabled(){return win.visible&&win.focused&&inputReady&&!navigationError&&state==='live'&&socket?.readyState===WebSocket.OPEN;}
 ...
 function connect(){if(stopped)return;state='opening';const ws=new WebSocket(`${location.protocol==='https:'?'wss:':'ws:'}//${location.host}/ws/stream/${encodeURIComponent(win.id)}`);socket=ws;
  ...
  ws.onmessage=async event=>{let data;try{data=JSON.parse(event.data);}catch{return;}
   ...
   if(data.type==='navigation'){navigationError=data.error||'';onnavigation(data);return;}
  ...
  ws.onclose=()=>{if(socket!==ws)return;release();onnavigation({canGoBack:false,canGoForward:false,busy:true});if(stopped)return;state='disconnected';message='Connection lost. Reconnecting; unsent input is discarded.';retryTimer=setTimeout(connect,Math.min(10000,500*2**attempt++));};
...
<div class="stream-wrapper">
 ...
 {#if navigationError||state!=='live'}<div class="stream-overlay" role="status"><span class="stream-glyph">{navigationError||state==='failed'?'!':'◌'}</span><strong>{navigationError?'Page navigation failed':state==='failed'?'Remote app unavailable':state==='disconnected'?'Stream disconnected':state==='paused'?'Stream paused':'Opening remote page…'}</strong><p>{navigationError||message||'The desktop remains local. This app runs in an isolated browser.'}</p>{#if state==='disconnected'}<small>Reconnecting automatically · no input replay</small>{/if}</div>{/if}
