...
<main class="desktop" bind:this={desktop} aria-label="Personal desktop">
 ...
 {:else if phase==='ready'}
  <nav class="shortcuts" use:reveal={{enabled:motion}} aria-label="Desktop apps">{#each apps as app,i (app.id)}<button class="shortcut" use:reveal={{enabled:motion,delay:Math.min(i,6)*25,kind:'slide'}} aria-label={'Open '+app.label} aria-describedby={showStatus?'health-'+app.id:undefined} disabled={busy===app.id} onclick={()=>open(app)} title={showStatus?statusDetail(observation(app)):app.description}><Icon name={app.icon||app.template||app.id}/><span>{app.label}</span><small>{app.mode==='stream'?'streamed':app.openMode==='tab'?'native · new tab':'native'}</small>{#if showStatus}<small id={'health-'+app.id} class="health"><i class:online={observation(app)?.state==='Online'} class:offline={observation(app)?.state==='Offline'}></i>{statusLabel(observation(app))}<span class="health-detail">{statusDetail(observation(app))}</span></small>{/if}</button>{/each}</nav>

  {#each windows as win,i (win.id)}<DesktopWindow {motion} {win} index={win.z??i} {areaWidth} {areaHeight} onchange={patch=>change(win.id,patch)} onfocus={()=>focus(win.id)} onminimize={()=>change(win.id,{visible:false,focused:false},true)} onclose={()=>close(win.id)} onreload={()=>reload(win.id)}/>{/each}
...
<footer><span class="maker">Made by Wicked Evil Incorporated</span>{#if phase==='ready'&&!onboarding}<nav class="dock" use:reveal={{enabled:motion,delay:90}} aria-label="Open windows"><button class="nav-toggle" aria-label="Navigation" aria-expanded={quick} onclick={()=>quick=!quick}>⠿</button>{#each windows as win}<button class:active={win.focused&&win.visible} class:minimized={!win.visible} aria-label={'Restore '+win.title} onclick={()=>focus(win.id)}><span class="dock-indicator">{win.visible?'▪':'▫'}</span>{win.title}</button>{/each}</nav><span class="footer-note"><span class="app-version">v{VERSION}</span> · {windows.filter(w=>w.visible).length} visible / {windows.length} open</span>{:else}<span class="footer-note"><span class="app-version">v{VERSION}</span> · {phase==='ready'&&onboarding?'first-run setup':phase==='loading'?'connecting':'account access required'}</span>{/if}</footer>
{#if quick}<section class="quick-nav" use:reveal={{enabled:motion,kind:'slide'}} aria-label="Navigation"><div class="panel-heading">Navigation <button aria-label="Close navigation" onclick={()=>quick=false}>×</button></div><p><span>{user?.displayName}</span> <small> / {user?.role}</small></p><p>APPS</p>{#each apps as app}<button onclick={()=>open(app)}><Icon name={app.icon||app.template||app.id}/><span>{app.label}<small>{app.kind==='web'?(app.mode==='stream'?'streamed · isolated browser':'native · '+(app.openMode==='tab'?'new tab':'desktop window')):(app.mode==='stream'?'streamed · synthetic lab':'native · built in')}</small></span><span>↗</span></button>{/each}<hr/><button onclick={()=>{settings='profile';quick=false;}}>Profile settings</button>{#if user?.role==='admin'&&!user?.mustChange}<button onclick={()=>{settings='admin';quick=false;}}>Control Panel</button>{/if}<button onclick={logout}>Sign out</button></section>{/if}
